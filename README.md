...
| `STT_MODEL` | 任意 | `tiny` | Whisper のモデル名 (`base`, `small` など) |
...
# (任意) Whisper 用のモデルをダウンロード (openai-whisper)
# python -m whisper --model tiny --download-only
```
...
| `/search notify mode:<on/off>` | 検索進捗読み上げの ON/OFF を切替 |
