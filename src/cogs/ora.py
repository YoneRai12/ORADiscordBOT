...
class ORACog(commands.Cog):
    ...
    async def _maybe_speak(
        ...
        *,
        ...
        if progress:
            ...
            # Default speech only if user prefers public replies.
    ...
    async def _handle_hotword(self, member: discord.Member, command_text: str) -> None:
        query = command_text
        if "調べ" in query:
            query = query.split("調べ", 1)[0]
        query = query.replace("を", " ").strip()
        if not query:
            logger.debug("Hotword detected but query empty")
            return
        if not self._search.enabled:
            await self._voice_manager.play_tts(member, "検索APIが未設定です。")
            return

        try:
            await self._voice_manager.play_tts(member, "Web検索を開始します")
            ...
            await self._voice_manager.play_tts(member, "検索が完了しました")
        except Exception:
            logger.exception("Hotword search failed")
            await self._voice_manager.play_tts(member, "検索に失敗しました")
    ...
    async def search_query(
        ...
        if member is not None:
            await self._maybe_speak(member, "Web検索を開始します", progress=True)

        try:
            results = await self._search.search(query)
        except Exception:
            logger.exception("Search failed", extra={"user_id": interaction.user.id})
            await interaction.followup.send("検索に失敗しました。", ephemeral=ephemeral_flag)
            if member is not None:
                await self._maybe_speak(member, "検索に失敗しました", progress=True)
            return
        ...
        if member is not None:
            await self._maybe_speak(member, "検索が完了しました", progress=True)

    @search_group.command(name="notify", description="検索進捗の読み上げを切り替えます。")
    @app_commands.describe(
        enabled="true で読み上げを有効化、false で無効化",
        ephemeral="true を指定するとエフェメラルで返信します",
    )
    ...
    async def image_ocr(
        ...
        try:
            ...
            await interaction.followup.send(str(exc), ephemeral=ephemeral_flag)
            return
